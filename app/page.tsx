...
export default function Page() {
  ...
  return (
    <div className="min-h-screen bg-[#0052FF] text-white flex flex-col items-center p-6 font-sans">
      ...
      <main className="w-full max-w-md bg-white/10 backdrop-blur-md rounded-3xl p-8 border border-white/20 shadow-2xl">
        <div className="mb-8 flex flex-col items-center text-center">
          ...
          <div className="bg-white/10 border border-white/10 rounded-xl px-4 py-2 mb-4 flex flex-col items-center shadow-inner min-w-[140px]">
            ...
            <span className="text-2xl font-mono font-extrabold text-white my-1">{txCount !== null ? txCount : "..."}</span>
            <span className="text-[8px] opacity-40 uppercase tracking-widest font-medium">Total Transactions</span>
          </div>
          
          <div className="space-y-4 mt-2">
            <p className="text-blue-50 text-md leading-relaxed">This Mini App is living proof that <span className="font-bold underline">anyone</span> can build onchain.</p>
            <div className="h-[1px] w-1/2 bg-white/20 mx-auto" />
            <p className="text-sm italic opacity-80">Our mission: empowering others to build.</p>
          </div>
        </div>

        <div className="space-y-4">
          <Transaction chainId={8453} calls={calls as any}>
            <TransactionButton className="w-full bg-white text-[#0052FF] font-bold py-4 rounded-2xl active:scale-95 transition-all shadow-lg shadow-blue-900/20" text="Check-in & Support" />
          ...
          {topFriends.length > 0 && (
            <div className="mt-6 pt-6 border-t border-white/10">
              <p className="text-white/60 text-[10px] mb-4 text-center uppercase tracking-[0.2em] font-bold">Invite your inner circle</p>
      ...
      <footer className="mt-auto pt-8 text-white/40 text-[9px] uppercase tracking-[0.2em] text-center">Built by a Solo Developer <br/> Base Network • Powered by OnchainKit</footer>
    </div>
  );
}